...
def main():
    ...
    lines += [
        ...
        f"новых карточек заблокировано без изображения: {kit.get('new_creation_blocked_without_images', '—')}",
        f"новых карточек заблокировано без закупочной цены: {kit.get('new_creation_blocked_without_purchase_price', '—')}",
