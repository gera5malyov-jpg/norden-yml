...
WEBASYST_VALUE = "336"
SKU_PREFIX = "DEEP-"
REPORT = HERE / "last_deephouse_webasyst_sync.json"
WRITE_DELAY = float(os.getenv("DEEPHOUSE_WEBASYST_WRITE_DELAY", "0.60"))
MONEY = Decimal("0.01")
...
def product_url(name, sku):
    ...
    base = text[:140].strip("-") or "deephouse"
...
def load_kit_module():
    path = ROOT / "deephouse-kit" / "sync_deephouse_kit.py"
    spec = importlib.util.spec_from_file_location("deephouse336_kit_sync_module", path)
...
def feature_code(title):
    return "deephouse336_" + hashlib.sha1(norm(title).encode("utf-8")).hexdigest()[:12]
...
def main():
    ...
    report = {
        ...
        "routing_characteristic": "Webasyst=336",
        "type_name": TYPE_NAME,
        "type_id": None,
    ...
    # Scan KIT only. Webasyst=336 is the routing gate; SKU is the exact match key.
    kit_by_sku = defaultdict(list)
    # Restrict the KIT scan to the managed SKU namespace first, then apply
    # Webasyst=336 as the authoritative routing gate.
    for variant in kit.list_all("/v1/variants", {"name": SKU_PREFIX}, "variants"):
        ...
        if char_value(variant, webasyst_char_id) != WEBASYST_VALUE:
            continue
        report["kit_webasyst_336_variants"] += 1
