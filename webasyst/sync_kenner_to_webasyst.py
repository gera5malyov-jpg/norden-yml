...
WRITE_DELAY = float(os.getenv("KENNER_WEBASYST_WRITE_DELAY", "0.60"))
MONEY = Decimal("0.01")
...
def load_kit_module():
    ...
    spec = importlib.util.spec_from_file_location("kenner_kit_sync_module", path)
...
def main():
    ...
    report = {
        ...
        "routing_characteristic": "Webasyst=334",
        "type_name": TYPE_NAME,
        "type_id": None,
    ...
    for variant in kit.list_all("/v1/variants", {}, "variants"):
        ...
        if char_value(variant, webasyst_char_id) != WEBASYST_VALUE:
            continue
        report["kit_webasyst_334_variants"] += 1
        ...
        if not sku.startswith(SKU_PREFIX):
            report["errors"].append({
                ...
                "sku": sku,
                "message": "Webasyst=334 variant does not use SKU prefix 334-",
            })
            continue
