#!/usr/bin/env python3
# create Ozon order in Dalli SPB basket
...
def dalli_post(root):
    token = s(os.environ.get("DALLI_TOKEN"))
    if not token:
        raise RuntimeError("Не задан GitHub Secret DALLI_TOKEN")
...
def choose_service(address_dict, services):
    ...
    raise RuntimeError("В договоре Dalli СПБ нет подходящего курьерского типа доставки")
