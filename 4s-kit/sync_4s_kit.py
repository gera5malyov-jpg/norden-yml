...
def main():
    ...
    force_media_refresh=s(os.getenv('FOURS_FORCE_MEDIA_REFRESH')).casefold() in {'1','true','yes','y'}
    ...
    report={
        ...
        'image_refresh_skipped_same_count':0,
    ...
    for n,o in enumerate(offers,1):
        ...
        for target in targets:
            ...
            # During the one-time forced cleanup, avoid re-uploading an already
            # matching gallery: the old parser typically produced extra related-
            # product images, which is visible as a media-count mismatch.
            if generated_333_variant(target) and o.get('images'):
                source_images=list(dict.fromkeys(
                    x for x in (o.get('images') or []) if s(x)
                ))[:10]
                current_images=[
                    x for x in (target.get('media') or [])
                    if s(x.get('type')).upper() in ('','IMAGE')
                ]
                needs_media=not current_images
                if force_media_refresh:
                    needs_media=len(current_images)!=len(source_images)
                    if not needs_media:
                        report['image_refresh_skipped_same_count']+=1
                if needs_media:
                    media=prepare_media(kit,o,report)
                    if media:
                        try:
