...
class SafeSession:
    ...
    def request_json(self, method, url, *, params=None, headers=None, json_body=None, files=None):
            ...
            try:
                ...
                if not response.ok:
                    ...
                    raise RuntimeError(
                        ...
                        f'{parsed.scheme}://{parsed.netloc}{parsed.path}{suffix}'
                    )
                ...
                try:
                    ...
                    parsed = urlsplit(url)
                    raise RuntimeError(
                        f'Invalid JSON from {parsed.scheme}://{parsed.netloc}{parsed.path}'
                    ) from exc
        ...
        parsed = urlsplit(url)
        raise RuntimeError(
            f'HTTP transport failure {method.upper()} '
    ...
    def download_to_file(self, url, path):
        ...
        for attempt in range(self.max_attempts):
            try:
                ...
                if not response.ok:
                    parsed = urlsplit(url)
                    raise RuntimeError(
                        f'HTTP {response.status_code} GET '
                        f'{parsed.scheme}://{parsed.netloc}{parsed.path}'
                    )
        ...
        parsed = urlsplit(url)
        raise RuntimeError(
            f'HTTP transport failure GET {parsed.scheme}://{parsed.netloc}{parsed.path}'
        ) from last_exc
