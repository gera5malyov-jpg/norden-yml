...
def build_parser():
    p=argparse.ArgumentParser(description='Samsonopt → Yandex KIT synchronizer'); p.add_argument('--dry-run',action='store_true'); p.add_argument('--skip-items',type=int,default=0); p.add_argument('--max-items',type=int,default=None); p.add_argument('--max-new',type=int,default=None); p.add_argument('--new-only',action='store_true'); p.add_argument('--report',default='samson-kit/state/last_sync.json'); return p

def main(argv=None):
    args=build_parser().parse_args(argv); settings=Settings.from_env(); http=SafeSession(max_attempts=10,timeout=(15,120)); samson=SamsonClient(settings.samson_api_key,http,settings.samson_base_url); kit=KitClient(settings.kit_token,http,settings.kit_base_url,min_request_interval=1.25); started=utc_now(); runner=SyncRunner(samson,kit,http,warehouse_name=settings.target_warehouse,dry_run=args.dry_run,skip_items=args.skip_items,max_items=args.max_items,max_new=args.max_new,new_only=args.new_only)
